...
Скрипт даляет `198.18.0.0/15` из `ipv4_exclude` в `S05xkeen`, включает DNS Override (`opkg dns-override`), перезапускает XKeen и сохраняет конфигурацию. Перед изменением создаёт бэкап файла.

**revert** — восстанавливает файл из бэкапа, выключает DNS Override, перезапускает XKeen и сохраняет конфигурацию.
